...
def HomePage(window):
    ...
    startButton = Button(
        ...
        text="Start",
        command=lambda: makeOrUse.set("make"),
        font=("helvetica", 30),
    )
    ...
    predictButton = Button(
        Home_Page.contentFrame,
        text="Predict",
        command=lambda: makeOrUse.set("use"),
        font=("helvetica", 30),
    )
...
def makeModel(window):
    ...
    for choice, description in problemTypeChoices.items():
        option = Radiobutton(
            ProblemSelect.contentFrame,
            text=choice,
            variable=problemType,
            value=choice,
            command=lambda description=description: explanationBox.config(
                text=description
            ),
        )
    ...
    nxtBtn = Button(
        ProblemSelect.contentFrame, text="next", command=lambda: continueVar.set(True)
    )
    ...
    while fileNotLoaded:
        ...
        try:
            ...
            filename = askopenfilename(
                title="Choose Training Data", filetypes=dataFiletypes
            )
            ...
            messagebox.showerror(
                title="Error", message=str(type(e)).split("'")[1] + str(e)
            )
            continue
        fileNotLoaded = False
    ...
    collectDataButton = Button(
        ...
        if len(featureListbox.listbox.get(0, "end")) > 0
        and len(targetListbox.listbox.get(0, "end")) > 0
        else messagebox.showwarning(
            title="Warning", message="You must have at least one feature and one target"
        ),
    )
    ...
    if "Text" in problemType.get():
        featureEncoder.fit(
            array(list(set(featureTrain.to_numpy().flatten().tolist())))
            .reshape(-1, 1)
            .ravel()
        )
    ...
    if "Classification" in problemType.get():
        targetEncoder.fit(
            array(list(set(targetTrain.to_numpy().flatten().tolist())))
            .reshape(-1, 1)
            .ravel()
        )
...
def useModel(window):
    ...
    while fileNotLoaded:
        ...
        try:
            counter += 1
            filename = askopenfilename(
                filetypes=[("Edward Machine Learning Creator Model", "*.emlcm")],
                title="Load Model",
            )
            ...
            if not (
                ("sklearn" in str(type(model)))
                and ("Classification" in problemType or "Regression" in problemType)
                and ("Text" in problemType or "Regression" in problemType)
                and ("LabelEncoder" in str(type(featureEncoder)))
                and ("LabelEncoder" in str(type(targetEncoder)))
                and ("pandas.core.indexes.base.Index" in str(type(featureColumns)))
                and ("pandas.core.indexes.base.Index" in str(type(targetColumn)))
            ):
                raise ValueError("File is invalid")
        except Exception as e:
            messagebox.showerror(
    ...
    while fileNotLoaded:
        ...
        try:
            counter += 1
            filename = askopenfilename(filetypes=dataFiletypes, title="Load Features")
            if path.splitext(filename)[1].lower() == ".csv":
    ...
    messagebox.showinfo(
        title="Save", message="Finished prediction. Saving results to file now"
    )
